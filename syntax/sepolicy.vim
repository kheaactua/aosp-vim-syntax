...
syn keyword def define

syn region stringLiteral start="`" skip="`" end="'"

" Attributes
syn region attrLine start=/attribute/ end=";" oneline
syn region attrLine start=/^\s*attribute/ end=";"
syn region attrLine start=/^\s*expandattribute/ end=";"
syn keyword attrKw contained attribute expandattribute containedin=attrLine

" Types
syn region typeLine start=/^\s*type/ end=";"

syn region typeAttrs contained start=",\s*" end="$" containedin=typeLine
syn keyword knownTypeAttrs contained domain coredomain exec_type file_type vendor_init system_file_type containedin=typeAttrs

syn region typeAlias contained start=/\valias\s+/ end=/,/ containedin=typeLine nextgroup=typeAttrs
syn keyword typeAliasDef contained alias containedin=typeAlias

syn match   typeDomain contained /[a-zA-Z_]\+/ containedin=typeLine nextgroup=typeAttrs,typeAlias skipwhite
syn keyword typeStart contained type containedin=typeLine nextgroup=typeDomain skipwhite
...
syn region ruleClass contained matchgroup=block start=/:\s*{/ end="}" containedin=ruleLine nextgroup=rulePerm skipwhite
...
syn keyword ruleStart contained allow neverallow type_transition allowxperm dontaudit containedin=ruleLine nextgroup=ruleDomain
...
hi typeAlias term=italic cterm=italic gui=italic
...
hi def link typeAliasDef    Keyword
hi def link typeAlias       Type
